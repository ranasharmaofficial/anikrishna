...
@section('title', 'Master Price List')
@section('content')
<style>
            ...
            <div class="col-md-12">
                <div class="card card-table">
					<div class="card-header d-flex align-items-center justify-content-between">
						<h5 class="card-title">Master Price</h5>
						<a href="{{ url('admin/add-master-price') }}" class="btn btn-primary ms-auto">Add Master Price</a>
					</div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
            ...
            <div class="form-group">
                <label for="edit_name">Price Range</label>
                <input type="text" name="price_range" id="edit_name" class="form-control" required>
            </div>
...
$(function () {
    ...
    $('#basicExampless').DataTable({
        processing: true,
        serverSide: true,
        ajax: '{{ route("admin.getMasterPrices.ajax") }}',
        columns: [
    ...
    $(document).on('click', '.editBtn', function () {
        let id = $(this).data('id');
        let price_range = $(this).data('price_range');
        let status = $(this).data('status');

        $('#edit_id').val(id);
        $('#edit_name').val(price_range);
    ...
    $('#editForm').submit(function (e) {
        ...
        $.ajax({
            url: "{{ route('admin.master_price.update') }}",
            method: "POST",
            data: formData,
